...
if $TEARDOWN; then
  ...
  oc delete clusterrolebinding rhbo-collector-traces-write rhbo-tempo-traces-reader rhbo-collector-logs-write rhbo-loki-application-reader --ignore-not-found 2>/dev/null || true
  oc delete clusterrole rhbo-tempo-traces-write rhbo-tempo-traces-read rhbo-loki-logs-write --ignore-not-found 2>/dev/null || true
...
log "Applying RBAC (ServiceAccount + Loki/Tempo ClusterRoles)..."
oc apply -f "$SCRIPT_DIR/collector/22-rbac.yaml"
